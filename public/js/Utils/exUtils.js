export class Utils {
    getProperties (dom) {                  // funcion que retorna un objeto con claves como id del dom
        let properties = {}
        ...
        domInputs.forEach(input => {
            ...
            if(isNaN(input.value)) properties[nameId] = input.value
            else                   properties[nameId] = Number(input.value)
        });
        return properties;
    }
    setProperties (dom, obj) {             // funcion que setea un dom con claves de un objeto
        const domInputs = dom.querySelectorAll('[idata]')
        domInputs.forEach(input => {
    ...
    printProperties (dom, obj) {           // funcion que prime texto al dom por claves - objeto
    ...
    outerHTML(inode) { return inode.outerHTML || new XMLSerializer().serializeToString(inode) }
    CamelCase (string) {
